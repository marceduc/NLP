...
public class parser3 {
	...
	public static void main(String[] args) {
		...
		LinkedHashMap<String, Integer> wLs = sortByValue(wL);
		...
		// Ausgabe: Woerterliste
		System.out.println("W�rter: " + (int) wordCount + " (" + wL.size() + " distinct)");
		
		// Ausgabe keys
		
		int len_stopwords = (int) (wordCount * 0.2);
		len_stopwords = 50;
		
		System.out.println("Stopwords: " + (int) len_stopwords );
		
		
		//wLs = wLs.submap(len_stopwords, wLs.size());
		
		Iterator<String> it = wLs.keySet().iterator();
		
		
		int i = 0 ;
		
		while (i < len_stopwords) {
		    it.next();
		    it.remove();
		    i += 1; 
		}
	...
	// Klasse zum Parsen nach Woerter
	public static class analyseAbs implements Runnable {
		...
		public void run() {
			...
			try {
				...
				for (File datei : paths) {
					System.out.println("Parse nach W�rter: " + datei.getName());
		...
		class UserHandler extends DefaultHandler {
			...
			public void startElement(String uri, String localName, String qName, Attributes attributes)
				...
				if (qName == "ArticleId" & attributes.getValue("IdType")=="pubmed" ) {
					ArticleId = true;
			...
			public void characters(char ch[], int start, int length) throws SAXException {
				...
				if (ArticleId) {
					...
					current_id = new String(ch, start, length).toLowerCase().replace(".", "");
